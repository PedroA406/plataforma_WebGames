// Pegar token e redirecionar se não existir
...
formCriarJogo.addEventListener('submit', async (e) => {
  ...
  if (!tipo) {
    mensagemJogo.style.color = 'red';
    mensagemJogo.textContent = 'Selecione o tipo de jogo.';
    return;
  }

  // Verifica se o tipo de jogo é suportado
  const tiposSuportados = ['quiz', 'memoria', 'associacao', 'verdadeiroFalso'];
  if (!tiposSuportados.includes(tipo)) {
    mensagemJogo.style.color = 'red';
    mensagemJogo.textContent = 'Tipo de jogo inválido.';
    return;
  }
